...
/*   Updated: 2018/09/26 17:35:00 by amaurel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			ft_hexfloat(t_buffer *buffer, t_conversion conv, t_argument arg)
{
	...
	p.len_mantissa = mantissa_length(p.mantissa, conv);
	...
	if (!ft_hexfloat_split1(buffer, conv, arg, &p)
		|| !ft_hexfloat_split2(buffer, conv, arg, &p)
		|| !ft_hexfloat_split3(buffer, conv, arg, &p))
		return (0);
	return (1);
}
